...
// Brand palette: deep charcoal, warm cream, brushed bronze accent.
const BRAND_DARK = "#141414";
const BRAND_CREAM = "#f1ead8";
const BRAND_ACCENT = "#a8814a";
...
function makeStripeTexture() {
  ...
  // Diagonal stripes ~35deg
  ctx.save();
  ctx.translate(size / 2, size / 2);
  ctx.rotate((35 * Math.PI) / 180);
  ctx.translate(-size, -size);
  const colors = [BRAND_DARK, BRAND_CREAM, BRAND_ACCENT];
  const stripeH = size / 10;
  for (let y = -size; y < size * 3; y += stripeH) {
    ctx.fillStyle = colors[Math.floor((y + size) / stripeH) % 3];
...
const ROT_SPEED = (Math.PI * 2) / 10; // one loop per 10s
...
function Pole({ animate }: { animate: boolean }) {
  ...
  useFrame((_, delta) => {
    if (!animate) return;
    const d = Math.min(delta, 0.05); // clamp to avoid jumps
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Front polished-glass highlight strip */}
